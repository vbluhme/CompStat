...
```{r}
### THIS CHUNK TAKES ABOUT 100ms to run. Calculate only once!
Phi <- splineDesign(c(rep(range(x), 3), x), x)
Omega <- pen_mat(x)
...
# Original evaluation
S1 <- Phi %*% solve(crossprod(Phi) + lambda_opt * Omega, t(Phi))
f1 <- S1 %*% y

# Decomposition, matrix multiplication
S2 <- U_tilde %*% solve(diag(length(x)) + lambda_opt*diag(Omega_tilde_svd$d), t(U_tilde))
f2 <- S2 %*% y

# Decomposition, but fast
f3 <- t(U_tilde) %*% y
f3 <- f3 / (1 + lambda_opt*Omega_tilde_svd$d)
f3 <- U_tilde %*% f3

# f2 and f3 are equal
range(f2-f3)

# We do not get same splines in corners!
p + geom_line(aes(y = f1), col = "red") + geom_line(aes(y = f2), col = "blue")
```

Why do they diverge? Because the first and last columns of $\mathbf S_\lambda$ diverge. Compare columns of matrices:

```{r, warning = FALSE, out.width = "100%"}
plot_cols <- function(S, col_vec) {
...
plot_cols(S1-S2, c(1:10, 138:147))
```
